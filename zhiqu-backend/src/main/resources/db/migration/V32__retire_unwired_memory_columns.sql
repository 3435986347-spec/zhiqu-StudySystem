...
--
-- 留下的两列 memory_epoch 已经真的接上了，但并没有按 V27 字面那道栅栏接
-- （V27 说「最终事务比对本列，不匹配则整体丢弃」，那会推翻三条既有判据）。
-- 为什么刻意不那么做，见 docs/adr/0001-v27-清空必须获胜-不按字面实现.md ——
-- 这个决定必然被下一个读 V27 的人重新提出，所以它需要一个比 git log 更容易撞见的落点。
--
-- 两列删，两列接：
--
