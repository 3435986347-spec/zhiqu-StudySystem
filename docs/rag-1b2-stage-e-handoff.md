...
| **3** | **回填打通** | **第一条转绿** ✅ 已实测（① 绿、③ 仍红） |
| **4** | **`sourceCount` 放宽** | **第三条转绿，集合归零** |

#### step 4 的三处判断（都不是机械改名）

1. **计数取自 `ScopeSelection`，不能从 `candidates` 反推。** 后者就是
   `distinct(sourceKey)` 已经在做的事，取 `max` 之后等于没放宽，③ 会继续红。
   这是安全的失败方向，而挡住它的正是夹具里那个**零候选的 Wiki 单元 B** ——
   「从结果反推范围」这条实现路径过不了它。
2. **数单元个数，不数命名空间个数。** 今天两者行为相同（只进一处 `> 1` 比较），
   选前者是因为它才是这个量的定义。javadoc 写明「目前只用于 `> 1`，改用途前回来看」——
   拿它分预算的那天，几百页 Wiki 时两种口径是 301 对 2，选错的表现是预算失衡不是异常。
3. **旧的口径断言反转，并补内建阳性对照。** `计数口径只含notebook资料` 钉的是 1B-1 的
   口径等价，放宽即其替代品已绿，故同提交反转。**旧夹具会让新断言空绿**：
   它直接插 `user_knowledge_page`，而范围读的是投影表，那个页根本进不了 `projectedUnits`，
   计数仍是 3 —— 通过的原因是「Wiki 压根没进来」。夹具改成经 `refreshUnitIfLive` 注册，
   并加一条 `scopedUnitCount() > notebookSourceIds().size()` 当阳性对照。

#### step 3 的两次夹具改动，期望值一个没动

照片类在 step 2 与 step 3 各改了一次**输入**：

| 步 | 改的东西 | 为什么是夹具而不是期望 |
|---|---|---|
| 2 | 状态行：`source_id`+资料哈希 → UNIT 行 + `canonical_hash` | `upsertUnitState` 真写的就是这种行 |
| 3 | 桩候选：`sourceId` → `namespace`+`unitId` | 真 sidecar 返回的就是这个（vector_store.py:243-244，返回体里没有 sourceId） |

**step 3 的失败信息自己证明了这一点**，不必靠推断：改夹具之前照片报的是
`expected [第1段,第0段,第2段] but was [第0段,第1段,第2段]` —— 那是 chunk_index 升序，
即 `legacyContextRows` 的顺序，说明候选<b>全被丢了</b>（缺 `unitId`），
而不是「向量路径把顺序搞错了」。同一份信息若只断言「有结果」，这两种情况分不开。

### 剩余的预期失败集合（换手时带走这一段）

step 2 实测：`Tests run: 196, Failures: 2, Errors: 0, Skipped: 0`，失败集合为

1. `RagRetrievalUnitDialectBaselineTest.wiki单元经向量路径进入候选集` → step 3 转绿 ✅ 已转
2. `RagRetrievalUnitDialectBaselineTest.每源配额按所有命名空间计数` → step 4 转绿

**step 3 之后剩余集合 = {第 2 条}**，即只剩「每源配额按所有命名空间计数」。
③ 在 step 3 后**仍红且必须仍红**已实测确认（`expected: <3>`）——
这一条是交接单事先写死的确认要求，理由是它此前红在一个不属于自己的原因上
（回填全丢 → 5 行关键词），若 step 3 让它转绿，说明夹具出了别的事而不是进展。
...
### step 3 带出来的一个规模问题，E-4 之前必须量一下

Wiki 范围是「该用户**全部** READY 的 WIKI_PAGE 单元」（`SourceScopeResolver`）——
Wiki 不按 Notebook 划分，今天也没有检索侧的「选中哪几页」语义，所以这是对的口径。
但它有两处随页数线性增长的东西，而现在**一处都没有量过**：

1. `payload.unitIds` 里会出现该用户每一个已索引的 Wiki 单元 —— 几百页就是几百个 id
   进一个 JSON 请求体，且 sidecar 侧变成 Chroma 的 `$in` 大列表；
2. `freshlyIndexedUnits` 的 `IN (...)` 状态查询同样按页数展开。

今天量不出来：`app.rag.enabled=false`，而演练库里的 Wiki 页是个位数。
**E-4 翻开关那一刻正是唯一能量的时刻**（也正是要重跑回滚演练、建新契约基线的那一刻），
两件事合并做。

### 上界已经先落地了，不等测量（因为等不起）

活壳无条件发 `includeWiki: true`（`assets/zhiqu-api.js:2930` 是硬编码字面量，不是用户开关），
所以开关一翻，每一次对话都会拉进该用户的整个 Wiki 语料，而**客户端没有下车口** ——
唯一的杠杆是把 `app.rag.enabled` 翻回 false，全局停。
「等量出曲线再定」不成立：量得到的时候已经全量暴露了。

已落地：`rag.wiki-scope-max`（`RuntimeFlagService`，默认 200，按 `updated_at` 倒序取最近 N 页），
现场可不重启收紧；截断记 `wikiScopeTruncations` + `wikiScopeMaxSeenUnits`（超出量，不只事件）。

**上界按页数不按 chunk 数**：token 成本已被 `min(max_candidate_k, candidateK)` 与
`finalK/maxContextChars/maxPerSource` 夹死且与范围大小无关；真正无界的是 id 基数，
它按单元个数增长。而按 chunk 给预算在巨页上是反的 —— 500 chunk 的页在 id 基数上只值 1，
...
| 全量对账 | 投影表填上 | `RagAdminService:183` 的守卫强制它必须先做 |
| 建代次 + 启用 | **检索行为真的改变** | 覆盖门禁：全部单元索引完才放行 |

**检索行为的改变点不是翻开关，是启用代次。**「部分索引的向量检索静默上线」这条路被堵住了。

写成 runbook 步骤是纪律，可以被跳过；`:183` 让跳过在启用那一刻响亮失败 —— 所以
**那个守卫是 E-4 顺序的唯一执行机制**，它的定义域必须覆盖全部命名空间（已修，见下）。

### 积压排空要分两下，用现成的 `worker-mode`

翻开关那一刻，钩子自 E-1a 起累积的 `UPSERT_UNIT` 全部变为可领。
每条要解密、清洗、切片、过 bge 嵌入、发出去，而同一台机器在服务用户；
除 `workerBatchSize` 与 1 秒轮询外**没有节流**。
（积压有界：`dedupe_key` 到终态才释放，所以是每页一条而不是每次编辑一条。）

1. `worker-mode=OFF` → 翻 `app.rag.enabled=true` → 确认没有别的东西因 `configured()` 变 true
   而改变行为（此时一条作业都不领）；
2. 再 `worker-mode=NORMAL`，看着 `rag_index_job` 的 PENDING 计数排空。

这样「翻开关」与「开始干活」是两个可观察事件，出问题能退回 `OFF` 而不必翻回 `enabled`
（后者会连带影响检索侧）。零新增代码 —— `RuntimeFlagService.WorkerMode.OFF` 本来就是为
cutover 建的（`RagIndexWorker:63` 直接 return）。

### E-4 要量的是排名分布，不是延迟曲线

200 是暂定值，没有依据。但**延迟 vs N 的曲线回答不了这个问题** —— 它对 N 单调上升，
读不出门槛，量完还是拍脑袋。

该量的是：**命中的 top-24 候选，在「按 `updated_at` 倒序」里的排名分布。**
sidecar 只回 24 条；若实测中命中几乎从不来自第 50 名之后的页，那么 N=200 安全、
N=50 也安全；若经常来自第 300 名，那 200 就在静默地砍掉有用的东西。
这个分布直接回答「超过哪个点之后，加大范围不再改变结果」。

成本与量延迟一样，产出却是一个有依据的 N。E-4 是唯一有真实语料的时刻。

**这项只能在启用代次之后取**（那时才有真实的 top-24），所以它是 E-4 的最后一步，
不能和翻开关并发做。

### 修了守卫的一个洞：`hasIndexableOrigin()` 看不见 Wiki

它此前只数 `ai_notebook_source` 的 READY 行。step 3 之后 Wiki 页也是可索引原料，于是
**只有 Wiki 页、没有资料的库**在对账之前：`units.isEmpty()` 且守卫为 false ⇒ 不抛 ⇒
启用一个空索引 ⇒ 覆盖检查以 0/0 空绿通过。分子与分母来自两张表，这次长在守卫上。

要紧之处在上面那张表：**这个守卫是 E-4 顺序的唯一执行机制**，它瞎掉等于第二行没人管。

修法上有一处方向判断值得记：**误报比漏报更难处理**。漏报是静默启用空索引；
而守卫若把系统页也当原料，它会抛「请先全量对账」，对账之后投影表**仍然是空的**
（系统页被排除），操作者卡死在一个响亮但无解的状态。所以：

- 排除规则走共享谓词 `RagNamespace.isExcludedWikiPage`，类型集作为共享常量传给 SQL，
  **不译成 SQL 副本**（译了就是第七份会漂移的词表）；
- 两个方向各一条用例：「只有 Wiki 页 ⇒ 不得启用」与「只有系统页 ⇒ 可以启用」。

> 记这一条的理由与「演练结果的有效性」同源：一个**只在开关翻开后才存在**的性质，
> 在开关翻开之前没有任何测试会提到它，于是它不会以「红」的形式出现，只会以线上表现出现。
...
> **预期失败集合归零，step 1 起的「失败集合恰好这些名字」判据在此终止，
> 健康判据回归「零失败」。**

这句话与那个 0 是<b>两件事</b>，都要留痕：0 证明序列走完，这句话证明判据换了回去。
集合变空只是事实，不构成宣告 —— 缺了它，分支上留下的状态是
「非空失败集合在这里是正常的」，而那句话曾经为真、之后为假，中间没有任何标记。
下一个看到红的人会去翻历史找「是不是又一次计划内的红」，而不是当场当成坏了。

演化实测：3 → 2 → 1 → 0（step 1 / 2 / 3 / 4），每一步的剩余集合都写在对应提交信息里。
...
**收尾挂在 E-3 上，而且是因果不是排期。** 前缀的存在条件是「`wikiContext` 与向量回填
同时产出 Wiki 行」；E-3 删掉 `wikiContext` 之后生产者只剩一个，该条件消失 ——
前缀防的事情不再可能发生。同一步里 `selectedWikiPageIds` 换语义，`:488` 那段匹配逻辑
本来就要动，拆字段（分桶用 `unitId`、匹配用请求侧 id）最便宜的时机正在那里。

> 「已知缺陷 + 已知修法 + 延期」在本仓库已有四次前科（`RECONCILE_UNITS`、`DELETE_SCOPE`、
> `DELETE_INDEX_VERSION`、fallback 指标）。得手的都挂在有触发条件的步骤上，
> 搁下的只有一句注释 —— 差别不在决心，在于「什么时候做」还依不依赖谁记得。
