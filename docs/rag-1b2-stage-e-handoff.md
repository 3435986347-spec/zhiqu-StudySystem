...
它钉的是候选行的<b>外部形状</b>，而且刻意不引用 `CandidateKeys`（引用了就两侧同源、
键名一改夹具跟着漂移，从此对键名改动完全不敏感）。所以
**`sourceId` → `unitId` 落地的那一刻它必然转红，那是设计内的正确结果。**

那一刻最省力、最自然的动作是「把期望里的键名改掉让它绿」——
而那正是整套替换流程从头到尾要防的那一个动作，且做的时候会觉得自己在修一个显然的破损。

| 三条新基准 | 含义 | 动作 |
|---|---|---|
| **已绿** | 新形状已被钉住，交接完成 | **退休**旧基准（删掉） |
| **仍红** | 在没有替代品的时候拆掉了旧的度量 | **回退** —— 改早了 |

把「退休」与「回退」分开是这一行的全部意义：两种情况下旧基准都是红的、看起来一模一样，
而正确动作相反。同一段判读也写在
`ContextBudgeterCharacterizationTest` 的类注释里 —— 它红的时候，人是在看那个文件，不是看这份交接单。
