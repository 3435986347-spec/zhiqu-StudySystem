...
一句话摘要：**三条新基准已绿 → 退休它；三条仍红 → 回退，改早了。**
两种情况下它都是红的、看起来一模一样，而正确动作相反。展开见那个文件。
...
**预期失败集合必须覆盖照片，不只覆盖新基准**：step 2 落地时 payload 那条也会红，
它若不在集合里，那个红就没有归属、读起来就是破坏。
歧义还在同一个类内部（三条里一条红是进展、另两条红是坏了），
所以判读同样写在该类的注释里：只有 payload 那条红 → 删掉它（替代品是新基准第二条）；
另外两条任意一条红 → 回退。
